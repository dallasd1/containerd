...
const testDmverityUUID = "11111111-1111-5111-8111-111111111111"
...
func TestFormatDmverityLayer(t *testing.T) {
	...
	t.Run("formats layer and creates metadata", func(t *testing.T) {
		...
		rootHash, err := d.formatDmverityLayer(ctx, layerPath, testDmverityUUID)
	...
	t.Run("skips formatting if metadata already exists", func(t *testing.T) {
		...
		// First format
		rootHash1, err := d.formatDmverityLayer(ctx, layerPath, testDmverityUUID)
		require.NoError(t, err)

		rootHash2, err := d.formatDmverityLayer(ctx, layerPath, testDmverityUUID)
		require.NoError(t, err)
	...
	t.Run("uses 4096-byte blocks in regular mode", func(t *testing.T) {
		...
		_, err := d.formatDmverityLayer(ctx, layerPath, testDmverityUUID)
		require.NoError(t, err)
	...
	t.Run("returns error for non-existent file", func(t *testing.T) {
		d := &erofsDiff{enableDmverity: true, enableTarIndex: false}
		_, err := d.formatDmverityLayer(ctx, filepath.Join(tmpDir, "missing.erofs"), testDmverityUUID)
		require.Error(t, err)
